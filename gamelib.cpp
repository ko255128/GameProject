...
void CInteger::SetIsLodBitmap(bool flag){
	isBmpLoaded = flag;
}
...
/////////////////////////////////////////////////////////////////////////////
// CInteger: 這個class提供顯示整數圖形的能力-------------------myself
// 1. 要懂得怎麼呼叫(運用)其各種能力，但是可以不懂下列的程式是什麼意思
// 2. 自己寫到運用CMovingBitmap的程式時，可以參考下列程式的寫法
/////////////////////////////////////////////////////////////////////////////
CMovingBitmap CIntegerST::digit[10];

CIntegerST::CIntegerST(int digits)
: NUMDIGITS(digits)
{
	isBmpLoaded = false;
}

void CIntegerST::Add(int x)
{
	n += x;
}

int CIntegerST::GetInteger()
{
	return n;
}

void CIntegerST::LoadBitmap()
{
	//
	// digit[i]為class varibale，所以必須避免重複LoadBitmap
	//
	if (!isBmpLoaded) {
		int d[10]={IDB_s0,IDB_s1,IDB_s2,IDB_s3,IDB_s4,IDB_s5,IDB_s6,IDB_s7,IDB_s8,IDB_s9};
		for (int i=0; i < 10; i++)
...
void CIntegerST::SetInteger(int i)
{
	n = i;
}

void CIntegerST::SetTopLeft(int nx, int ny)		// 將動畫的左上角座標移至 (x,y)
{
	x = nx; y = ny;
}

void CIntegerST::ShowBitmap()
{
	//注意! 不支援負數顯示
	GAME_ASSERT(isBmpLoaded, "CIntegerST: 請先執行LoadBitmap，然後才能ShowBitmap");
	int nx;		// 待顯示位數的 x 座標
	int MSB;	// 最左邊(含符號)的位數的數值
	if (n >= 0) {
		MSB = n;
...
void CGameState::ShowInitProgress(int percent)
{
	...
	CBrush *pb, b(RGB(255,255,255));				// 畫綠色 progress框
...
: NUM_GAME_STATES(7)
{
	...
	gameStateTable[GAME_STATE_RUN2]  = new CGameStateRun2(this);
